...
const GOOGLE_SHEET_CSV_URL = 'https://docs.google.com/spreadsheets/d/e/2PACX-1vSekTVRYingSPdcpjSzVazPOJRJr_SxAyQV1lvqvgzvoW6BtnP8aEoVOo1sZSF6tJ27tLXv7JvxCPP9/pub?output=csv';
...
export async function fetchStallData(): Promise<Record<string, string>[]> {
    ...
    try {
        ...
        console.warn(`Initial fetch with timestamp failed with status: ${response.status}. Retrying without timestamp.`);
    } catch (error) {
        // Catches network errors (e.g., DNS, CORS).
        console.warn('Initial fetch with timestamp failed with a network error. Retrying without timestamp.', error);
    }
