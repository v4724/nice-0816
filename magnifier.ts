...
export function createMagnifier(
  ...
  const updateRowIndicator = () => {
    ...
    const lensCenterX_pct =
      ((magnifierWrapper.offsetLeft + magnifierWrapper.offsetWidth / 2) /
        mapWidth) *
      100;
    const lensCenterY_pct =
      ((magnifierWrapper.offsetTop + magnifierWrapper.offsetHeight / 2) /
        mapHeight) *
      100;
    ...
    if (
      lensCenterY_pct < TOP_VISIBLE_BOUNDARY_Y ||
      lensCenterY_pct > BOTTOM_VISIBLE_BOUNDARY_Y
    ) {
